...
class BaseballElimination:
    ...
    def isEliminated(self, teamName):
        result = []
        num = len(self.teams)
        idx = self.team2id[teamName]
        teamWin = self.wins[idx] + sum(self.against[idx])
        for i in range(num):
            if self.teams[i] == teamName:
                continue
            if self.wins[i] > teamWin:
        ...
        for i in range(num):
            if i == idx:
                continue
            aaa.addEdge(FlowEdge(game + i + 1, v - 1, teamWin - self.wins[i]))
            for j in range(i + 1, num):
                if j == idx:
                    continue
                else:
                    aaa.addEdge(FlowEdge(0, check, self.against[i][j]))
                    aaa.addEdge(FlowEdge(check, game + i + 1, float('inf')))
                    aaa.addEdge(FlowEdge(check, game + j + 1, float('inf')))
                check += 1
        fff = FordFulkerson(aaa, 0, v - 1)
        for i in range(num):
            if self.teams[i] == teamName:
                continue
            if fff.inCut(game + i + 1):
                result.append(self.teams[i])
        if len(result) > 0:
            return True, result
        return False, []
